...
Vue.createApp({
    ...
    computed: {
        filtered() {
            return this.articles.filter(article => article.title.toLowerCase().includes(this.searchQuery.toLowerCase()));
        }
    }
}).mount("#app"); // приложение Vue нужно монтировать в элемент id="app"